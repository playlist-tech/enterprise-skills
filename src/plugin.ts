 ...
 *
 * A plugin is a curated bundle of skills, described by a `plugin.yaml` that
...
// Display name for the CLI in help/error text, and the API base for plugin
...
async function runPluginInstall(args: string[]): Promise<void> {
  ...
  if (!rawSource) {
    ...
    console.error(pc.dim(`Usage: ${CLI_NAME} plugin install <org>/<repo>@<plugin-name>`));
    process.exit(1);
  }
...
async function runPluginList(): Promise<void> {
  ...
  if (byPlugin.size === 0) {
    ...
    console.log(pc.dim(`Install one with: ${CLI_NAME} plugin install <org>/<repo>@<plugin-name>`));
    return;
  }
...
async function runPluginRemove(args: string[]): Promise<void> {
  ...
  if (!pluginName) {
    console.error(pc.red(`Usage: ${CLI_NAME} plugin remove <plugin-name>`));
    process.exit(1);
  }
...
async function runPluginUpdate(args: string[]): Promise<void> {
  ...
  const rawSource = source[0];
  if (!rawSource) {
    console.error(pc.red(`Usage: ${CLI_NAME} plugin update <org>/<repo>@<plugin-name>`));
    process.exit(1);
  }
...
async function runPluginSearch(args: string[]): Promise<void> {
  ...
  const url = `${API_BASE}/api/plugins/search${query ? `?q=${encodeURIComponent(query)}` : ''}`;
  ...
  if (res.status === 404) {
    ...
    console.log(
      pc.dim(`You can still install a known plugin: ${CLI_NAME} plugin install <org>/<repo>@<name>`)
    );
    return;
  }
  if (!res.ok) {
  ...
  console.log();
  console.log(pc.dim(`To install, run: ${CLI_NAME} plugin install <org>/<repo>@<name>`));
}

function showPluginHelp(): void {
  console.log(`
Usage: ${CLI_NAME} plugin <subcommand> [options]

Bundle and install curated groups of skills.
...
Examples:
  ${CLI_NAME} plugin install org/repo@my-plugin
  ${CLI_NAME} plugin list
  ${CLI_NAME} plugin remove my-plugin
`);
}
